...
import { Alert, Button, ButtonGroup, Container, Pagination, Table } from 'react-bootstrap'
...
function Dashboard() {
  ...
  const fetchForms = async (page = 1, query = search) => {
    ...
    const res = await axios.get(
      `http://localhost:3500/users?page=${page}&limit=10&search=${query}`
    );
                          ...
                          <tr key={form._id}>
                            ...
                            <td>
                              <ButtonGroup size="sm" aria-label="Basic example">
                                <Button size="sm" onClick={() => navigate(`/form-load/${form._id}`)} variant="outline-primary">View</Button>
